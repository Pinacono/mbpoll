...
// -----------------------------------------------------------------------------
      ...
      for (arg = optind + 1; arg < argc; arg++, i++) {
        ...
        switch (ctx.eFunction) {
          ...
          case eFuncHoldingReg:
            if ( ctx.eFormat == eFormatInt32 || ctx.eFormat == eFormatUint32 ) {
            ...
            else if ( ctx.eFormat == eFormatInt16 || ctx.eFormat == eFormatUint16 ) {
  ...
  if (ctx.bIsReportSlaveID) {
    ...
    // int32 et float utilisent 2 registres 32 bits
    iNbReg = ( ( ctx.eFormat == eFormatInt32 ) || ( ctx.eFormat == eFormatUint32 ) || (ctx.eFormat == eFormatFloat)) ?
...
vPrintReadValues (int iAddr, int iCount, xMbPollContext * ctx) {
  ...
  for (i = 0; i < iCount; i++) {
    ...
    switch (ctx->eFormat) {
      ...
      case eFormatByte:
        uint8_t v = (unsigned char) DUINT8 (ctx->pvData, i);
        printf ("%u (0x%02x)", v, v);
        iAddr++;
        break;

      case eFormatInt16: {
        int16_t v_int16 = (int16_t) DUINT16 (ctx->pvData, i);
        /*
        if (v_int16 & 0x8000) {
          printf ("%u (%d)", v_int16, (int) (int16_t) v_int16);
        }
        else {
          printf ("%u", v);
        }
        */
        printf ("%d (0x%04X)", v_int16, v_int16);
        iAddr++;
      ...
      case eFormatUint16:
        uint16_t v_uint = (uint16_t) (DUINT16 (ctx->pvData, i));
        printf ("%u (0x%04X)", v_uint, v_uint);
        iAddr++;
        break;
      ...
      case eFormatInt32:
        int32_t v_int32 = lSwapLong (DINT32 (ctx->pvData, i));
        printf ("%d (0x%08X)", v_int32, v_int32);
        iAddr += 2;
        break;

      case eFormatUint32:
        uint32_t v_uint32 = (uint32_t) lSwapLong (DINT32 (ctx->pvData, i));
        printf ("%u (0x%08X)", v_uint32, v_uint32);
        iAddr += 2;
        break;
...
vAllocate (xMbPollContext * ctx) {
  ...
  switch (ctx->eFunction) {
    ...
    case eFuncInputReg:
    case eFuncHoldingReg:
      if ( (ctx->eFormat == eFormatInt32) || (ctx->eFormat == eFormatUint32) || (ctx->eFormat == eFormatFloat) ) {
        // Registres 32-bits
        ulDataSize *= 4;
      }
      else {
...
//          01234567890123456789012345678901234567890123456789012345678901234567890123456789
           ...
           "  -t 3:byte     16-bit input register data type with single byte (8 bits) dislay\n"
...
#endif
           "  -t 4          16-bit output (holding) register data type with signed int display\n"
           "  -t 4:byte     16-bit output (holding) register data type with single byte (8 bits) display\n"
           ...
           "  -t 4:int32    32-bit integer data type in output (holding) register table\n"
